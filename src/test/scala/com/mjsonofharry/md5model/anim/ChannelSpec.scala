...
class ChannelSpec
  ...
  it should "still work when the channel has strings" in {
    Given("a channel with strings")
    val channel = CHANNEL_STRINGS
  ...
  it should "still work when the channel has no range values" in {
    Given("a channel with no range values")
    val channel = CHANNEL_NORANGE
